...
| dependency | build-time | runtime | built if not available | comments |
|------------|------------|---------|------------------------|----------|
| [4ti2](https://4ti2.github.io/) | ✅ | ✅ | ✅ | Used by the `FourTiTwo` package |
| [Autoconf](https://www.gnu.org/software/autoconf/) | ✅ | ❌ | ❌ | Called by `autogen.sh` to generate the `configure` script |
| [Automake](https://www.gnu.org/software/automake/) | ✅ | ❌ | ❌ | Called by `autogen.sh` on some older systems where `config.guess`, `config.sub`, and `install-sh` are not generated by `autoconf`.  Needed for building many other dependencies. |
| [GNU Bison](https://www.gnu.org/software/bison/) | ✅ | ❌ | ❌ | Generates parsers for the d language and html (for checking broken links in the documentation) |
| [Boehm-Demers-Weiser Garbage Collector](https://www.hboehm.info/gc/) |  ✅ | ✅ | 📌 | Garbage collector.  Built by default (use `--with-system-gc` to override) |
| [Boost](https://www.boost.org/) | ✅ | ❌ | ❌ | Stack traces, regular expressions, and some special math functions |
| [cddlib](https://github.com/cddlib/cddlib) | ✅ | ✅ | ✅ | Build dependency for Gfan and TOPCOM.  Used by the `Polyhedra` package. |
| [CSDP](https://github.com/coin-or/Csdp) | ✅ | ✅ | ✅ | Used by the `SemidefiniteProgramming` and `SumsOfSquares` packages |
| [Eigen](https://libeigen.gitlab.io/) | ✅ | ❌ | ✅ | Linear algebra |
| [FFLAS-FFPACK](https://linbox-team.github.io/fflas-ffpack/) | ✅ | ❌ | ✅ | Linear algebra over finite fields (header-only, but requires linking against Givaro) |
| [flex](https://github.com/westes/flex) |  ✅ | ❌ | ❌ | Generates html lexer for checking broken links in the html documentation |
| [FLINT](https://flintlib.org/) | ✅ | ✅ | ✅ | Used extensively throughout the system.  On older systems, [Arb](https://arblib.org/) is also required.  |
| [GCC](https://gcc.gnu.org/) | ✅ | ❌ | ❌ | Compiler for C, C++, and Fortran.  [Clang](https://clang.llvm.org/) is also supported.  |
| [Gfan](https://users-math.au.dk/~jensen/software/gfan/gfan.html) | ✅ | ✅ | ✅ | Used by the `gfanInterface` package |
| [Git](https://git-scm.com/) | ✅ | ❌ | ❌ | Clone Macaulay2 repository and the repositories of several of its dependencies |
| [Givaro](https://casys.gricad-pages.univ-grenoble-alpes.fr/givaro/) | ✅ | ✅ | ✅ | Dependency of FFLAS-FFPACK |
