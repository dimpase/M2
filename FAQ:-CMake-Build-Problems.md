...
When compiling with several versions of `readline` installed (such as when using [Gentoo Prefix](https://wiki.gentoo.org/wiki/Project:Prefix)), compilation may fail at the linking step. Use the CMake variable <code>READLINE_ROOT_DIR</code> to tell CMake where to find the right version of <code>readline</code>. The specified directory should contain the file <code>include/readline/readline.h</code>. In the case of Gentoo Prefix, one should run:
```
